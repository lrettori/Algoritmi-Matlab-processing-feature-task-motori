...
%     patientCode = 0;
    ...
    if strcmp(exerCode,"THFFv")
        exerCode = "THFv";
    elseif strcmp(exerCode,"THFFa")
        exerCode = "THFa";
    elseif strcmp(exerCode,"OPCLv")
        exerCode = "OPCv";
    elseif strcmp(exerCode,"OPCLa")
        exerCode = "OPCa";
    end

    switch taskSelected
        case {"FTAP","THFF","OPCL","PSUP","KINT","THFFv","STUP","THFFa", "OPCLv", "OPCLa"}
            limbs = "one arm";

        case {"POST","HRST"}
            limbs = "two arms";

        case {"TTHP","HEHE","HTTP","HETO","ROTA"}
            limbs = "one leg";
    ...
    if strcmp(limbs,"one arm") || strcmp(limbs,"one leg")
        ...
        if strcmp(limbs,"one arm")
            if strcmp(side, 'Lx')
                dataOut(:,1:6) = dataIn.data(:,columnOffset + 2 : columnOffset + 7);
                dataOut(:,7:12) = dataIn.data(:,columnOffset + 11 : columnOffset + 16);
                dataOut(:,13:18) = dataIn.data(:,columnOffset + 20 : columnOffset + 25);
                latoForFileName = "SX";
                latoForHeader = "Sx";

            elseif strcmp(side,'Rx')
                dataOut(:,19:24) = dataIn.data(:,columnOffset + 2 : columnOffset + 7);
                dataOut(:,25:30) = dataIn.data(:,columnOffset + 11 : columnOffset + 16);
                dataOut(:,31:36) = dataIn.data(:,columnOffset + 20 : columnOffset + 25);
                latoForFileName = "DX";
                latoForHeader = "Dx";
            end
        ...
        elseif strcmp(limbs,"one leg")
            if strcmp(side, 'Lx')
                dataOut(:,37:42) = dataIn.data(:,columnOffset + 2 : columnOffset + 7);
                latoForFileName = "SX";
                latoForHeader = "Sx";

            elseif strcmp(side,'Rx')
                dataOut(:,43:48) = dataIn.data(:,columnOffset + 2 : columnOffset + 7);
                latoForFileName = "DX";
                latoForHeader = "Dx";
            end
        end
    ...
    elseif (strcmp(limbs,"two arms") || strcmp(limbs,"two legs")) && strcmp(side,'Lx')
        ...
        switch limbs
            case "two arms"
                dataOut(1:samplesLeft,1:6) = dataInLeft.data(:,columnOffsetLeft + 2 : columnOffsetLeft + 7);
                dataOut(1:samplesLeft,7:12) = dataInLeft.data(:,columnOffsetLeft + 11 : columnOffsetLeft + 16);
                dataOut(1:samplesLeft,13:18) = dataInLeft.data(:,columnOffsetLeft + 20 : columnOffsetLeft + 25);
                dataOut(1:samplesRight,19:24) = dataInRight.data(:,columnOffsetRight + 2 : columnOffsetRight + 7);
                dataOut(1:samplesRight,25:30) = dataInRight.data(:,columnOffsetRight + 11 : columnOffsetRight + 16);
                dataOut(1:samplesRight,31:36) = dataInRight.data(:,columnOffsetRight + 20 : columnOffsetRight + 25);
            case "two legs"
                dataOut(1:samplesLeft,37:42) = dataInLeft.data(:,columnOffsetLeft + 2 : columnOffsetLeft + 7);
                dataOut(1:samplesRight,43:48) = dataInRight.data(:,columnOffsetRight + 2 : columnOffsetRight + 7);
        end
    ...
    elseif strcmp(exerCode,"GTAH") && strcmp(side,"Lx")
        ...
        dataOut(1:samplesLeftHand,1:6) = dataInLeftHand.data(:,columnOffsetLeftHand + 2 : columnOffsetLeftHand + 7);
        dataOut(1:samplesLeftHand,7:12) = dataInLeftHand.data(:,columnOffsetLeftHand + 11 : columnOffsetLeftHand + 16);
        dataOut(1:samplesLeftHand,13:18) = dataInLeftHand.data(:,columnOffsetLeftHand + 20 : columnOffsetLeftHand + 25);
        dataOut(1:samplesRightHand,19:24) = dataInRightHand.data(:,columnOffsetRightHand + 2 : columnOffsetRightHand + 7);
        dataOut(1:samplesRightHand,25:30) = dataInRightHand.data(:,columnOffsetRightHand + 11 : columnOffsetRightHand + 16);
        dataOut(1:samplesRightHand,31:36) = dataInRightHand.data(:,columnOffsetRightHand + 20 : columnOffsetRightHand + 25);
        dataOut(1:samplesLeftFoot,37:42) = dataInLeftFoot.data(:,columnOffsetLeftFoot + 2 : columnOffsetLeftFoot + 7);
        dataOut(1:samplesRightFoot,43:48) = dataInRightFoot.data(:,columnOffsetRightFoot + 2 : columnOffsetRightFoot + 7);
    ...
    if flagSaveFile
        ...
        % % Formato cella, per corretto distanziamento tra colonne adiacenti
        % cellDataOutput = {};
        % 
        % % Header
        % cellDataOutput{1,1} = 'Date'; 
        % cellDataOutput{1,2} = '23/10/2023';
        % cellDataOutput{2,1} = 'Time'; 
        % cellDataOutput{2,2} = '11:40:00';
        % cellDataOutput{3,1} = 'Codice Paziente'; 
        % cellDataOutput{3,2} = patientCode;
        % cellDataOutput{4,1} = 'Codice esercizio'; 
        % cellDataOutput{4,2} = exerCode;
        % cellDataOutput{5,1} = 'Lato'; 
        % cellDataOutput{5,2} = latoForHeader;
        % cellDataOutput{6,1} = 'Ripetizione'; 
        % cellDataOutput{6,2} = rep;
        % cellDataOutput{7,1} = 'N. Samples';
        % cellDataOutput{7,2} = samples;
        % cellDataOutput{8,1} = 'Versione';
        % cellDataOutput{8,2} = '1.0';
        % 
        % % Colonna con i tempi
        % for ii = 1 : length(timeVectorString)
        %     cellDataOutput{9 + ii, 1} = timeVectorString(ii, :);
        % end
        % 
        % % Prima riga
        % for jj = 1 : 49
        %     cellDataOutput{9, jj} = ColumnNames{jj};
        % end
        % 
        % % Dati
        % for ii = 1 : length(timeVectorString)
        %     for jj = 1 : 48
        %         cellDataOutput{9 + ii, 1 + jj} = dataOut(ii, jj);
        %     end
        % end
        % 
        % writecell(cellDataOutput,title,'delimiter','\t');
