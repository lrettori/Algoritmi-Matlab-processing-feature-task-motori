...
filenameData = 'D:\Ricerca UNIFI\Olimpia\Dati\Dati per prove debug\HEHE_DX_Ex2 - Copia.txt';
firstImportRawData = importdata(filenameData);
% Estraggo le sole colonne di mio interesse
data_ax = firstImportRawData.data(:,43);
data_ay = firstImportRawData.data(:,44);
data_az = firstImportRawData.data(:,45);
data_wx = firstImportRawData.data(:,46);
data_wy = firstImportRawData.data(:,47);
data_wz = firstImportRawData.data(:,48);

% Setto altri parametri di inizializzazione (DA CONTROLLARE)
directory = 'D:\Ricerca UNIFI\Olimpia\Dati\Dati per prove debug';
filename = 'HEHE_DX_Ex1 - Copia';
